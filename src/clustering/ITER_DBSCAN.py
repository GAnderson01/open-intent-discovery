...
from clustering.sentenceEmbedding import getEmbeddings
import re
...
class ITER_DBSCAN(DBSCAN):
    ...
    def __init__(self, initial_distance=0.10, initial_minimum_samples=20, delta_distance=0.01, delta_minimum_samples=1,
                 max_iteration=5, threshold=300, features=None, gpu=False, max_mb_per_batch=None
                 ):
        ...
        self.labels_ = None
        self.gpu = gpu
        self.max_mb_per_batch = max_mb_per_batch
    ...
    def compute(self, data):
        ...
        for i in range(self.max_iteration):
            ...
            if self.gpu:
                cluster_labels = cuDBSCAN(eps=self.initial_distance,
                                          min_samples=self.initial_minimum_samples,
                                          metric='precomputed',
                                          max_mbytes_per_batch=self.max_mb_per_batch)
            else:
